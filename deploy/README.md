...
- **GET** `node/documents-file-stream?driveId=…&itemId=…` — streams file bytes through Dolibarr (Graph app token). Fallback when anonymous links are disabled; works with **Files.Read.All** (no write permission required). The Join SPA loads PDFs from this endpoint (not `fetch()` to `*.sharepoint.com`) to avoid browser **CORS** errors.
