...
const addressLines = computed(() => {
  const parts = []
  const addr = String(props.venue?.address ?? props.venue?.location ?? '').trim()
  if (addr) parts.push(addr)
  const zip = String(props.venue?.zip ?? '').trim()
  const country = String(props.venue?.country ?? '').trim().toUpperCase()
  if (zip || country) {
    parts.push([zip, country].filter(Boolean).join(' '))
  }
  return parts
})
        ...
        >
          ...
          <div class="venue-detail-modal-body">
            ...
            <aside class="venue-detail-modal-side">
              ...
              <section v-if="addressLines.length" class="venue-detail-block">
                <h3 class="venue-detail-block-title"><I18nText k="venues.detailAddress" /></h3>
                <p v-for="(line, i) in addressLines" :key="'addr-' + i" class="venue-detail-text">{{ line }}</p>
              </section>

              <section v-if="venue.supportInfo" class="venue-detail-block">
                <h3 class="venue-detail-block-title"><I18nText k="venues.detailSupport" /></h3>
                <p class="venue-detail-text">{{ venue.supportInfo }}</p>
              </section>

              <section v-if="contacts.length" class="venue-detail-block">
                <h3 class="venue-detail-block-title"><I18nText k="venues.detailContact" /></h3>
              ...
              <p v-if="!contacts.length && !addressLines.length && !venue.supportInfo" class="venue-detail-text-muted">
